...
/**
  ...
  */
		...
		//sha1digest
		static inline int8_t SHA1Digest( SHA1Context *context, uint8_t Message_Digest[SHA1HashSize])
		{
			uint8_t i;
		...
		//sha1update
		static inline int8_t SHA1Update(SHA1Context *context, const uint8_t *message_array, uint32_t length)
		{
			if (!length){
				return shaSuccess;
			}
		...
		static inline void KDF(uint8_t *Kp, int32_t K_len, uint8_t *Zp)
		{
			int32_t len, i;
		...
		static inline void hmac_sha1(uint8_t *text, int32_t text_len, uint8_t *key, int32_t key_len, uint8_t *digest)
		{
			SHA1Context context;
