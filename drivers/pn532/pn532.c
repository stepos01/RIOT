...
#define PN532_ACKSIZE 6
uint8_t pn532ack[PN532_ACKSIZE] = {0x00, 0x00, 0xFF, 0x00, 0xFF, 0x00};
#define PN532_NACKSIZE 6
uint8_t pn532nack[PN532_NACKSIZE] = {0x00, 0x00, 0xFF, 0xFF,0x00, 0x00};
...
// #define PN532_SS            GPIO(1,3)    //pin 23 on the GPIO
// #define PN532_SSGPIO        1           //if you want to use GPIO to control SS/CS/NSS
// #define PN532_GPIO_0_EN 1           //Enable GPIO_0. pin 23 on the GPIO
// #define SPI_0_EN    1           //Enable SPI_0
// #define spi_cs      0

static int spi_dev = 0;         //SPI_0
static gpio_t spi_cs = 0;
 ...
 */
/**************************************************************************/
void pn532_init_master(pn532_t *dev, spi_t spi, spi_speed_t spi_speed, gpio_t cs_pin)
{
    dev->spi = spi;
    dev->cs_pin = cs_pin;
    dev->spi_speed = spi_speed;

    LOG_DEBUG("SPI_%i not initialized as master, cs: GPIO_%i, mode: %i, speed: %i\n",
              spi, (int)cs_pin, SPI_CONF_FIRST_RISING, spi_speed);

    int res;
    res = spi_init_master( spi, SPI_CONF_FIRST_RISING, spi_speed );
    if (res < 0) {
        LOG_ERROR("SPI_init_master: error initializing SPI_%i device (code %i)\n", spi, res);
        }
    res = gpio_init(cs_pin, GPIO_DIR_OUT, GPIO_PULLUP);
    if (res < 0){
        LOG_ERROR("GPIO_init_out: error initializing GPIO_%i as CS line (code %i)\n", (int)cs_pin, res);
    }
    /* Set the given GPIO pin to HIGH */
    pn532_ss_off();
    LOG_DEBUG("SPI_%i successfully initialized as master, cs: GPIO_%i, mode: %i, speed: %i\n",
              spi, (int)cs_pin, SPI_CONF_FIRST_RISING, spi_speed);
    spi_dev = dev->spi;
    spi_cs = dev->cs_pin;

    for (int i = 0; i<10; i++) gpio_toggle(dev->cs_pin);
    gpio_clear(dev->cs_pin);
    spi_transfer_byte(dev->spi, 0xaa, NULL);
    spi_transfer_byte(dev->spi, 0xaa, NULL);
    spi_transfer_byte(dev->spi, 0xaa, NULL);
    spi_transfer_byte(dev->spi, 0xaa, NULL);
    gpio_set(dev->cs_pin);
...
uint8_t pn532_spi_write(uint8_t p)
{
    ...
    if (res == 1){
        ...
        #endif
        LOG_DEBUG("Transferred byte on the SPI bus with value: %X\n", p);
    }
#endif
...
uint8_t pn532_spi_transfer_bytes(uint8_t *buffer, uint8_t blen)
{
    ...
    uint8_t n = spi_transfer_bytes(spi_dev, (char *)buffer, NULL, blen);
...
uint8_t pn532_spi_read(void)
{
    ...
    res = spi_transfer_byte(spi_dev, 0, (char *)&in);
    ...
    if ( res == 1 ){
            ...
            printf("[Reverse-bit-order] ");
        #endif
        LOG_DEBUG("Read on SPI bus value %X\n", in);
    }
...
{
    LOG_DEBUG("Wake up PN532 Shield\n");
    ...
    LOG_DEBUG("Writebuffer and readbuffer cleared...\n");
...
uint8_t pn532_spi_write_command(uint8_t *buff, uint8_t* cmd, uint8_t cmdlen)
{
    ...
    LOG_DEBUG("Sending command to PN532: ");
    ...
    LOG_DEBUG("writing the frame to PN532...\n");

    /* Sending constructed frame from host to PN532 */
...
uint16_t pn532_read_pn( uint8_t *buffer )
{
    ...
    do{
        ...
        while (!pn532_is_ready()) {
                timeout--;
                if (0 == timeout) {
                    LOG_ERROR("Time out when waiting for ACK\n");
...
#ifdef DEVELHELP
    LOG_DEBUG( "Here it is the read buffer.\n" );
    for( uint8_t i=0; i < len; i++){
        LOG_DEBUG( "| [%i]: %X |", i, buffer[i] );
    }
    LOG_DEBUG( "\n" );
#endif
...
{
    pn532_ss_on();
    uint8_t ackbuff[PN532_ACKSIZE];
    /* Write Data Read = 0x03 */
    pn532_spi_write(PN532_SPI_DATAREAD);

    for(int i=0; i<PN532_ACKSIZE; i++){
    ...
    LOG_DEBUG("ACK frame:\n");
    for(int i=0; i<PN532_ACKSIZE; i++){
        LOG_DEBUG("| %X |", ackbuff[i]);
    }
    LOG_DEBUG("END ACK FRAME\n");
#endif
    puts("I got: ");
    for (int i = 0; i<PN532_ACKSIZE; i++) printf("%x ", ackbuff[i]);
    puts("");
    puts("Should: ");
    for (int i = 0; i<PN532_ACKSIZE; i++) printf("%x ", pn532ack[i]);
    puts("");
...
#ifdef DEVELHELP
    LOG_DEBUG("Writing the ACK frame to PN532\n");
#endif
...
void pn532_get_general_status(void)
{
    ...
    LOG_DEBUG("Get the general status of the PN532.\n");
    ...
    LOG_DEBUG("Frame response of GetGeneralStatus:\n");
    for(int i=0; i < len; i++){
        LOG_DEBUG("| [%i] %X |", i, readbuffer[i]);
    }
    LOG_DEBUG("End General Status frame.\n");
#else
    (void) len;
#endif
}
 ...
 *  @params     picc    Emulation of a ISO/IEC144443-4 PICC is enabled
 *
...
uint8_t pn532_set_parameters( uint8_t nad, uint8_t did, uint8_t autatr, uint8_t autrats, uint8_t picc )
{
    ...
    if( len != 2 ) {
        LOG_DEBUG("Status incorrect! Status value: %X", readbuffer[2]);
...
uint8_t pn532_format_to_ndef( uint8_t * url, uint8_t ndefprefix, uint8_t blockNumber) {
    ...
    //                  uint8_t keya[6] = {0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56};
                    ...
                    success = pn532_mifareclassic_write_ndef_uri( 1, ndefprefix, (char *)url );
                    if (success) {
